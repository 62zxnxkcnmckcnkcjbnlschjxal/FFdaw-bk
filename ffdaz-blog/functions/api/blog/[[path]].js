...
export async function onRequestGet(ctx) {
  try {
    ...
    if (id && id !== 'blog') {
      ...
      if (!authed && article.status === 'draft') return json({ ok: false, error: '未授权' }, 401);
    ...
    if (!authed) {
      if (statusFilter === 'draft') return json({ ok: false, error: '未授权' }, 401);
      if (!statusFilter) statusFilter = 'published'; /* 未授权默认只读已发布 */
    }
    let list = d.articles.slice();
    if (statusFilter) {
      list = list.filter(function (a) { return a.status === statusFilter; });
    }
    if (!authed) list = list.filter(function (a) { return !a.hidden; });
